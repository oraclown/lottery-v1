...
TICKETS: constant(uint256) = 10 ** 6

total_tickets: public(uint256)
ticket_buyers: public(address[TICKETS])
...
@external
def __init__(
    _total_tickets: uint256,
    _buy_period: uint256, # in seconds
    _ticket_price: uint256,
    _admin_fee: uint256,
):
    assert _total_tickets <= TICKETS, "too many tickets"
    ...
    self.total_tickets = _total_tickets
...
@payable
def buy_ticket():
    assert self.tickets_bought < self.total_tickets, "no more tickets available"
...
@external
def get_ticket_buyers() -> address[TICKETS]:
